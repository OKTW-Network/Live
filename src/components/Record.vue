<template>
    <div @click="emitClick" class="RecordDiv">
        <img class="RecordThumb" v-bind:src="record.thumbSrc"/>
...
export default {
  ...
  computed: {
      getPublishTime(){
        ...
        return(`${twoNum(this.record.publishTime.getFullYear())}-${twoNum(this.record.publishTime.getMonth() + 1)}-${twoNum(this.record.publishTime.getDate())} ${twoNum(this.record.publishTime.getHours())}:${twoNum(this.record.publishTime.getMinutes())}:${twoNum(this.record.publishTime.getSeconds())}`);
      },
      getDuration(){
